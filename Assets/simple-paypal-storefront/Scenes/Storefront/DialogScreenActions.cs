...
public class DialogScreenActions : MonoBehaviour {
	...
	public void HideDialogScreen() {
		...
		dialogTitle.text = "";
		dialogMessage.text = "";
		OkButton.gameObject.GetComponentInChildren<TMP_Text> ().text = "OK";
	...
	public void setContextStoreOpen() {
		...
		OkButton.gameObject.GetComponentInChildren<TMP_Text> ().text = "Close";
	}

	public void setContextStoreLoadFailure() {

		dialogContext = DialogContext.STORE_LOAD_FAILURE;
		dialogTitle.text = "Connection Error";
		dialogMessage.text = "Please Try again Later.";
		OkButton.gameObject.SetActive (true);
		NoButton.gameObject.SetActive (false);
		YesButton.gameObject.SetActive (false);
